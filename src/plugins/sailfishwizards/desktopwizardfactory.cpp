    ...
    if (!isEmpty) {
        ...
        for (int i = 0; i < translations.size(); i++) {
            if (translations.at(i).getTranslation() != "") {
                ...
                const auto languageCodeMap = td.getLanguageCodeMap();
                if (languageCodeMap.contains(translations.at(i).getLanguage())) {
                    translationsStr += "Name[";
                    translationsStr += languageCodeMap.value(translations.at(i).getLanguage());
                    translationsStr += "]=" +translations.at(i).getTranslation() + "\n";
                }
            }
        }
